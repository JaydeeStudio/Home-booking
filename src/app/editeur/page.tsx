...
export default function EditeurPage() {
  ...
  const handleSaveSpace = async (space: any) => {
    const { error } = await supabase.from("spaces").update({
      name: space.name, capacity: space.capacity, description: space.description, color: space.color, image_url: space.image_url
    }).eq("id", space.id);
    if (!error) triggerSuccess(); else alert("Erreur : " + error.message);
  };
    ...
    <div className="flex h-[100dvh] bg-gray-50 font-sans overflow-hidden">
      ...
      <aside className="w-72 bg-white border-r border-gray-200 flex flex-col shadow-sm z-10 shrink-0 hidden md:flex">
        <div className="p-8 border-b border-gray-100">
          <h1 className="text-2xl font-black uppercase tracking-tight text-gray-900 leading-tight">
            <span className="block">Site</span><span className="block text-indigo-600 text-sm">Panneau d'édition</span>
          </h1>
        </div>
        <div className="p-4 flex-1 flex flex-col gap-2 overflow-y-auto">
          <button onClick={() => setActiveTab("textes")} className={`flex items-center p-4 rounded-2xl font-bold transition-all ${activeTab === "textes" ? "bg-indigo-50 text-indigo-700 shadow-sm border border-indigo-100" : "text-gray-500 hover:bg-gray-100"}`}>
            <FileText size={20} className="mr-4" /> Textes & CGV
          </button>
          <button onClick={() => setActiveTab("espaces")} className={`flex items-center p-4 rounded-2xl font-bold transition-all ${activeTab === "espaces" ? "bg-indigo-50 text-indigo-700 shadow-sm border border-indigo-100" : "text-gray-500 hover:bg-gray-100"}`}>
            <LayoutTemplate size={20} className="mr-4" /> Les Salles
          </button>
        </div>
        <div className="p-6 border-t border-gray-100 bg-gray-50/50">
          <button onClick={() => supabase.auth.signOut()} className="w-full flex items-center justify-center p-4 rounded-2xl font-bold text-red-600 hover:bg-red-50 transition-colors border border-transparent hover:border-red-100"><LogOut size={20} className="mr-3" /> Déconnexion</button>
        </div>
      </aside>
            ...
            <div className="animate-in fade-in duration-300">
              ...
              <form onSubmit={handleSaveContent} className="space-y-8">
                <div className="bg-white p-8 rounded-[32px] shadow-sm border border-gray-200">
                  <h3 className="font-black uppercase text-xs tracking-widest text-indigo-500 mb-6 flex items-center"><span className="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center mr-3">1</span> Page d'accueil</h3>
                  <div className="space-y-6">
                    <div><label className="block text-xs font-bold text-gray-700 mb-2">Titre principal</label><input type="text" required value={content.intro_title} onChange={(e) => setContent({...content, intro_title: e.target.value})} className="w-full border border-gray-200 rounded-xl p-4 bg-gray-50 font-bold focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all" /></div>
                    <div><label className="block text-xs font-bold text-gray-700 mb-2">Paragraphe d'explication</label><textarea required value={content.intro_paragraph} onChange={(e) => setContent({...content, intro_paragraph: e.target.value})} className="w-full border border-gray-200 rounded-xl p-4 bg-gray-50 font-medium focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none h-32 resize-none transition-all leading-relaxed" /></div>
                  </div>
                </div>

                <div className="bg-white p-8 rounded-[32px] shadow-sm border border-gray-200">
                  <div className="flex justify-between items-start mb-6">
                    <h3 className="font-black uppercase text-xs tracking-widest text-indigo-500 flex items-center"><span className="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center mr-3">2</span> Conditions Générales (CGV)</h3>
                  ...
                  <div><label className="block text-xs font-bold text-gray-700 mb-2">Texte complet</label><textarea required value={content.cgv_text} onChange={(e) => setContent({...content, cgv_text: e.target.value})} className="w-full border border-gray-200 rounded-xl p-4 bg-gray-50 font-medium focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none h-80 resize-none transition-all leading-relaxed" /></div>
                </div>

                <button type="submit" className="w-full bg-indigo-600 text-white font-black uppercase tracking-widest py-5 rounded-2xl hover:scale-[1.02] transition-transform flex items-center justify-center shadow-xl shadow-indigo-600/20 text-sm"><Save size={20} className="mr-3" /> Enregistrer toutes les modifications</button>
              </form>
            </div>
          )}
                ...
                {spaces.map(space => (
                  <div key={space.id} className="bg-white p-6 md:p-8 rounded-[32px] shadow-sm border border-gray-200 relative overflow-hidden">
                    ...
                    <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-8 border-b border-gray-100 pb-6 gap-4">
                       ...
                       <button onClick={() => handleSaveSpace(space)} className="bg-gray-900 text-white px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest flex items-center justify-center hover:bg-indigo-600 transition-colors shadow-lg"><Save size={16} className="mr-2" /> Enregistrer la salle</button>
                    </div>
                    
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                      <div className="space-y-5">
                        <div><label className="block text-[10px] uppercase tracking-widest font-black text-gray-400 mb-2">Nom</label><input type="text" value={space.name} onChange={(e) => updateSpaceState(space.id, 'name', e.target.value)} className="w-full border border-gray-200 rounded-xl p-3 bg-gray-50 font-bold outline-none focus:bg-white focus:ring-2 focus:ring-gray-900 transition-all" /></div>
                        <div className="grid grid-cols-2 gap-4">
                          <div><label className="block text-[10px] uppercase tracking-widest font-black text-gray-400 mb-2">Capacité</label><input type="number" value={space.capacity} onChange={(e) => updateSpaceState(space.id, 'capacity', e.target.value)} className="w-full border border-gray-200 rounded-xl p-3 bg-gray-50 font-bold outline-none focus:bg-white focus:ring-2 focus:ring-gray-900 transition-all" /></div>
                          <div><label className="block text-[10px] uppercase tracking-widest font-black text-gray-400 mb-2">Couleur</label>
                          ...
                          {space.image_url && (
                            <div className="flex gap-3 mt-4 overflow-x-auto pb-2 scroll-smooth">
                              {space.image_url.split(',').map((url: string) => url.trim()).filter(Boolean).map((url: string, idx: number) => (
                                 <div key={idx} className="relative w-24 h-24 shrink-0 group rounded-xl overflow-hidden border border-gray-200 shadow-sm">
                      ...
                      <div className="flex flex-col">
                         ...
                         <textarea value={space.description || ""} onChange={(e) => updateSpaceState(space.id, 'description', e.target.value)} className="w-full border border-gray-200 rounded-xl p-4 bg-gray-50 font-medium outline-none focus:bg-white focus:ring-2 focus:ring-gray-900 text-sm flex-1 min-h-[250px] resize-none transition-all leading-relaxed" placeholder="Décrivez l'ambiance, le mobilier..." />
                      </div>
                    </div>
                  </div>
                ))}
      ...
      {showSuccess && (
        <div className="fixed bottom-8 right-8 z-50 bg-gray-900 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center font-bold animate-in slide-in-from-bottom-5 fade-in duration-300">
          <CheckCircle2 className="text-green-400 mr-3 w-6 h-6" /> Modifications enregistrées !
        </div>
      )}
    </div>
  );

...
export default function AdminPage() {
    ...
    const processAutoValidation = async () => {
      if (user && bookings.length > 0) {
        ...
        if (action === 'validate' && id) {
           ...
           // On s'assure qu'on ne valide pas 2 fois la même chose
  ...
  const updateStatus = async (id: string, status: 'confirmed' | 'rejected') => {
    ...
    // Si on refuse, on supprime carrément la ligne de la base de données
    if (status === 'rejected') {
      const { error } = await supabase.from("bookings").delete().eq("id", id);
      if (!error) {
        await notifyUser('DELETED', booking);
        // Mise à jour de l'état local INSTANTANÉE pour retirer le bloc
    ...
    if (!error) {
      ...
      setSelectedBooking(null); 
      fetchBookings(); // Rafraîchit tout
    }
  };

  // SAUVEGARDE UNE MODIFICATION ADMIN
  ...
  // Fonction utilitaire pour centraliser l'envoi d'email

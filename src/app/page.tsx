...
export default function Home() {
  ...
  const selectDate = (day: Date) => {
    if (!isBefore(day, today)) {
      setCurrentDate(day);
      if (window.innerWidth < 1024) setIsSidebarOpen(false); // Fix mobile
    }
  };
          ...
          <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100">
            ...
            <div className="grid grid-cols-7 gap-1">
              {calendarDays.map((day, i) => {
                ...
                return (
                  <div key={i} onClick={() => selectDate(day)}
            ...
            <div className="mt-4 pt-4 border-t border-gray-200 text-center">
              <button onClick={() => {setCurrentDate(today); setCurrentMonthView(today);}} className="text-xs font-bold text-gray-500 hover:text-black transition">
              ...
              <table className="w-full border-separate border-spacing-0 min-w-[800px]">
                <thead>
                  <tr className="sticky top-0 z-30">
                    <th className="sticky left-0 z-50 bg-gray-100 border-b border-r border-gray-200 w-16 lg:w-20 h-16 shadow-[2px_2px_0_rgba(0,0,0,0.02)]">
                      <Clock size={16} className="mx-auto text-gray-400" />
                    </th>
                    {spaces.map(space => (
                      <th key={space.id} className="bg-white/95 backdrop-blur-md border-b border-r border-gray-200 h-16 px-2">
                        <span className="text-[10px] lg:text-xs font-black uppercase tracking-widest block truncate" style={{ color: space.color }}>{space.name}</span>
                      </th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {hours.map(h => (
                    <tr key={h} className="h-16">
                      <td className="sticky left-0 z-20 bg-gray-50 border-r border-b border-gray-100 text-[10px] font-bold text-gray-400 text-center shadow-[2px_0_0_rgba(0,0,0,0.02)]">
                        {h}:00
                      </td>
                      {spaces.map(space => {
                        ...
                        return (
                          <td key={space.id} className="border-r border-b border-gray-100 relative p-0 h-16 group">
                            <div onClick={() => !isOccupied && !isPast && handleSlotClick(space.id, h)} 
                                 className={`w-full h-full transition-colors flex items-center justify-center ${isOccupied || isPast ? 'bg-gray-50/50 cursor-not-allowed' : 'cursor-pointer hover:bg-gray-50'}`}>
                            ...
                            {spaceBookings.filter(b => new Date(b.start_time).getHours() === h).map(b => (
                              <div key={b.id} className={`absolute inset-x-1.5 z-10 rounded-xl p-2 text-[10px] font-black text-white truncate shadow-sm pointer-events-none transition-all ${b.status === 'pending' ? 'opacity-60 border-dashed border-gray-400' : 'opacity-90'}`} 
                                   style={{ top: '4px', height: `calc(${(new Date(b.end_time).getHours() - new Date(b.start_time).getHours()) * 64}px - 8px)`, backgroundColor: space.color, borderColor: b.status === 'pending' ? 'transparent' : 'rgba(0,0,0,0.1)' }}>
                                {b.user_name}
          ...
          <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleBookingSubmit} className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div><label className="block text-[10px] font-black text-gray-400 uppercase mb-1.5">Prénom</label><input type="text" required placeholder="Jean" value={formData.first_name} onChange={(e) => setFormData({...formData, first_name: e.target.value})} className="w-full border rounded-xl p-3.5 font-medium outline-none focus:ring-2 focus:ring-black" /></div>
                <div><label className="block text-[10px] font-black text-gray-400 uppercase mb-1.5">Nom</label><input type="text" required placeholder="Dupont" value={formData.last_name} onChange={(e) => setFormData({...formData, last_name: e.target.value})} className="w-full border rounded-xl p-3.5 font-medium outline-none focus:ring-2 focus:ring-black" /></div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div><label className="block text-[10px] font-black text-gray-400 uppercase mb-1.5">Téléphone</label><input type="tel" required placeholder="+41 79 123 45 67" value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} className="w-full border rounded-xl p-3.5 font-medium outline-none focus:ring-2 focus:ring-black" /></div>
              </div>

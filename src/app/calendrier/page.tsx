...
// GENERATEUR 15 MIN POUR L'ADMIN
...
export default function AdminPage() {
  ...
  const updateStatus = async (id: string, status: 'confirmed' | 'rejected') => {
    ...
    if (error) {
      if (error.message.includes("prevent_double_booking")) {
        setErrorMessage("Ce créneau chevauche une autre réservation ou un blocage déjà validé pour cette salle. Vérifiez bien les horaires.");
      } else {
  ...
  const generateBlocks = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const endRecDate = recurrenceOption === "none" ? currentDay : new Date((document.querySelector('input[name="end_recurrence"]') as HTMLInputElement)?.value || currentDay);
    const blocks: any[] = []; 
    let limit = 0; 

    while (currentDay <= endRecDate && limit < 365) {
      const st = new Date(currentDay); const [sh, sm] = blockData.start_time.split(':'); st.setHours(parseInt(sh), parseInt(sm), 0);
      const et = new Date(currentDay); const [eh, em] = blockData.end_time.split(':'); et.setHours(parseInt(eh), parseInt(em), 0);
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[100dvh] bg-gray-50 font-sans overflow-hidden relative">
      ...
      {/* MODAL BLOCAGE MULTIPLE AVEC SELECTEURS (CHOIX 2) */}
      {showBlockModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[100] p-4" onMouseDown={(e) => {if(e.target === e.currentTarget) setShowBlockModal(false)}}>
          <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 p-8 font-sans border border-white/20">
            ...
            <form onSubmit={generateBlocks} className="space-y-4">
              <div><label className="text-[10px] font-black uppercase text-gray-400 block mb-1">Espace à bloquer (Optionnel ici)</label><select name="space_id" className="w-full border border-gray-200 rounded-xl p-3 bg-gray-50 font-bold focus:bg-white outline-none cursor-pointer"><option value="">Utilisez les boutons ci-dessous</option>{spaces.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
              <div><label className="text-[10px] font-black uppercase text-gray-400 block mb-1">Nom du créneau</label><input type="text" name="block_name" value={blockData.title} onChange={e => setBlockData({...blockData, title: e.target.value})} placeholder="Ex: Célébrations" required className="w-full border border-gray-200 rounded-xl p-3 bg-gray-50 font-bold focus:bg-white outline-none" /></div>
              <div><label className="text-[10px] font-black uppercase text-gray-400 block mb-1">Date de début</label><input type="date" name="start_date" value={format(blockDate, "yyyy-MM-dd")} onChange={e => setBlockDate(new Date(e.target.value))} required className="w-full border border-gray-200 rounded-xl p-3 bg-gray-50 font-bold focus:bg-white outline-none" /></div>
              
              {/* SELECTEURS CHOIX 2 (15 MIN) */}
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black uppercase text-gray-400 block mb-1">De</label>
                  <select 
                ...
                <div>
                  <label className="text-[10px] font-black uppercase text-gray-400 block mb-1">À</label>
                  <select 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select name="recurrence" value={recurrenceOption} onChange={(e) => setRecurrenceOption(e.target.value)} className="w-full border border-gray-200 rounded-xl p-3 bg-gray-50 font-bold focus:bg-white outline-none cursor-pointer">
              ...
              <div>
                <label className="text-[10px] font-black text-gray-400 uppercase mb-3 block mt-2">Salles à bloquer simultanément</label>
          ...
          <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 p-8 font-sans max-h-[90vh] overflow-y-auto border border-white/20">
             ...
             <form onSubmit={handleEditSave} className="space-y-6">
                {isEditing ? (
                  <>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="text-[10px] font-black uppercase text-gray-400 block mb-1">Fin</label>
                        <select 
                          required 
                          className="w-full border border-gray-200 rounded-xl p-3 bg-gray-50 font-bold focus:bg-white outline-none cursor-pointer" 
                ...
                <div className="grid grid-cols-3 gap-4 pt-4 border-t border-gray-100">
                  <button type="button" onClick={() => deleteBooking(selectedBooking.id)} className="p-4 bg-red-50 text-red-600 rounded-2xl font-black uppercase tracking-widest text-[10px] flex flex-col items-center hover:bg-red-100 transition"><Trash2 className="w-5 h-5 mb-2"/> Supprimer</button>
